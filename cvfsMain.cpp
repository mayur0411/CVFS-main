...
void InitialiseSuperBlock()
{
	...
	SUPERBLOCKobj.TotalInodes = MAXINDOE;
	SUPERBLOCKobj.FreeInode = MAXINDOE;
}
...
void ls_file()
{
	...
	if(SUPERBLOCKobj.FreeInode == MAXINDOE)
	{
		printf("Error : There are no files\n");
		return;
	}
	printf("\nFile_Name\tInode Number\tFile Size\tLink Count\n");
	printf("-----------------------------------------------------------\n");
...
int main()						// (customised shell) 
{
	...
	while(1)
	{
		...
		if(count == 1)
		{
			...
			{
				printf("(Terminating the Pranav's File System)\n");
				break;
			}
			else
			{
		...
		else if(count == 3)
		{
			if(strcmp(Command[0],"create") == 0)
			{
				...
				if(ret == -1)
					printf("ERROR : Incorrect parameters\n");
				if(ret == -2)
					printf("ERROR : File not present/found\n");
				if(ret == -3)
					printf("ERROR : Permission denied\n");
				if(ret == -4)
